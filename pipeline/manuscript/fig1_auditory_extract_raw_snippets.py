"""One-time NAS extraction for Fig 1 (auditory variant)'s raw-voltage snippets.

Fig 1 auditory variant (pipeline/manuscript/fig1_auditory.py -- see that
file's docstring for why this is a variant, not the current primary Fig1)
shows two short raw-voltage windows (magnetic-stimulus "null" panel +
white-noise "positive" panel) plus 100 randomly-sampled raw spike waveforms,
all for a single exemplar unit. Those are the only pieces of the whole
figure that need NAS access -- everything else (spike times, Fourier stats)
comes from the pipeline's own data/20230415.nwb. Rather than have
fig1_auditory.py query the NAS live every time it's regenerated, this script
extracts those snippets once (with NAS access) into untracked .npy files
under data/fig1_raw_auditory/, which fig1_auditory.py then loads directly.
(A separate directory from the primary fig1.py's data/fig1_raw/ -- both
scripts happen to use the filename "mag_trace.npy", so sharing a directory
would silently clobber whichever ran second.)

Run manually, offline, whenever the exemplar unit/window constants below
change (they're kept in sync with fig1_auditory.py by importing them from it):

    python pipeline/manuscript/fig1_auditory_extract_raw_snippets.py
...
from pipeline.manuscript.fig1_auditory import (
    AGGREGATED_PATH,
    CLUSTER_ID,
    EXPERIMENT,
    MAG_CONTINGENCY,
...
OUT_DIR = Path(__file__).resolve().parents[2] / "data" / "fig1_raw_auditory"
